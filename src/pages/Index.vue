<template>
  <q-page>
    ...
    <div class="relative flex min-h-screen flex-col justify-center overflow-hidden bg-gray-50">
      <transition appear enter-active-class="animated rubberBand infinite">
        <p class="mx-auto flex items-end text-gray-500">Don't need to refresh *</p>
      </transition>
      <transition-group appear enter-active-class="animated fadeInRightBig slow">
          ...
          <q-card>
            <q-card-section>
              ...
              <div class="grid sm:flex sm:flex-nowrap">
                ...
                <q-btn :disable="!historyPrice || historyPrice.length < 10" label="Search" class="mt-4 ml-0 sm:mt-0 sm:ml-4 bg-gray-800 text-white text-capitalize" @click="onGetHistoryCryptoPrice(cryptoData.name, historyPrice)" />
              </div>
              <p class="mt-4" v-if="price"><code class="text-xs sm:text-sm font-bold text-gray-800 cursor-pointer hover:underline" style="text-decoration-style: wavy">{{ cryptoData.name }}</code> price on <code class="text-xs sm:text-sm font-bold text-gray-800 whitespace-nowrap cursor-pointer hover:underline" style="text-decoration-style: wavy">{{ historyPrice }}</code>: <code class="text-xs sm:text-sm font-bold text-gray-800 whitespace-nowrap cursor-pointer hover:underline" style="text-decoration-style: wavy">U$ {{ price }}</code></p>
            </q-card-section>
          </q-card>
